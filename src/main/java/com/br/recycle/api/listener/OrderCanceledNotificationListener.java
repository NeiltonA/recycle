...
public class OrderCanceledNotificationListener{
	...
	@Value("${recycle.sendgrid.template.cancel.Id}")
	private String templateIde;
	

    @TransactionalEventListener// fase especifica que o evento pode ser disparado
    public void onCancelOrder(DonationCancelEvent event) throws IOException {
    	...
    	private static class DynamicTemplatePersonalization extends Personalization {
    		...
    		public void addDynamicTemplateData(Donation donation) {
    			if (dynamic_template_data == null) {
    				...
    				String date = donation.getDateCancellation().toString();
    				OffsetDateTime dt = OffsetDateTime.parse(date);
    				DateTimeFormatter fmt = DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss");
    				String dateform = fmt.format(dt);
    				dynamic_template_data.put("dateCancellation", dateform);
    				if(donation.getStatus().name() == "CANCELED") {
    					dynamic_template_data.put("status", "Cancelado");
    				}
    			} else {
